...
let ansPIN = await inquirer.prompt(
                                        {name: "askPIN", type: "number", message: "Enter PIN"}
                                   )
...
//If fast cash option is selected     
                                   if(options.operation === "Fast Cash"){
                                        let enterpsAmount = await inquirer.prompt(
                                             {name:"psamount", type:"list", message:"Enter Amount", choices:["2000", "4000", "6000", "8000", "10000"]}
                                             )
...
//If entered amount axceedes available balance
                                       ...
                                       if (myBalance >= 0){console.log(`Your remaining balance is ${myBalance}`)}
                                        else {console.log("Your transuction can't be processed, please enter valid amount")}
                                   }
//If Balance Inquiry option is selected
                                   else (options.operation === "Balance Inquiry");{
